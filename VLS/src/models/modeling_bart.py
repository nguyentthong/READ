...
class READ(nn.Module):
    def __init__(self, in_channels, adapter_channels=64, kernel_size=3):
        super().__init__()
        self.fc1 = nn.Linear(in_channels, adapter_channels)
        self.rnn = nn.RNN(adapter_channels, adapter_channels, 1)
    ...
    def forward(self, x, T):
        ...
        x = self.fc1(x)
        x_input = x
        x_input = self.rnn(x_input)[0]
        x = self.fc2(x_input)
        x_id += x
        return x_id

class PvlaBaseModelOutput(BaseModelOutput):
    last_hidden_state: torch.FloatTensor = True
    hidden_states: Optional[Tuple[torch.FloatTensor]] = True
    attentions: Optional[Tuple[torch.FloatTensor]] = True
    ot_loss: torch.FloatTensor = True
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None,
                 fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=None, dim_common=256, n_attn_heads=1, p=2):
        ...
        if cross_attn_type == 0:
            ...
            self._recurrent_adapter = READ(text_feature_dim + visual_feature_dim, 4)
            if use_forget_gate:
    ...
    def calc_pvla_loss(self, hidden_states, image_features):
        m = hidden_states.shape[1]
        n = image_features.shape[1]

        C = self.cost(hidden_states, image_features)
        distance_list = []
        for i in range(len(C)):
            current_C = C[i]
            a = torch.ones(m, 1)/m
            b = torch.ones(n, 1)/n
            distance = partial_ot(current_C, a, b)
    ...
    def forward(
            ...
            if self.training and (dropout_probability < self.layerdrop): 
                ...
                if idx == self.fusion_layer:
                    if self.cross_attn_type == 0:
                        ...
                        output = self._recurrent_adapter(torch.cat((hidden_states, image_features), 2), 2)
                        output = self._linear_2(output)
        ...
        if self.training:
            pvla_loss = self.calc_pvla_loss(hidden_states, image_features_transformed)

        if not return_dict:
            return tuple(v for v in [hidden_states, encoder_states, all_attentions] if v is not None)

        if self.training:
            pvla_output = PvlaBaseModelOutput(
                last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions,
            )
            pvla_output.pvla_loss = pvla_loss
            return pvla_output
...
class PvlaSeq2SeqModelOutput(Seq2SeqModelOutput):
    last_hidden_state: torch.FloatTensor = True
    ...
    ot_loss: torch.FloatTensor = None
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        pvla_seq2seq_model_output = PvlaSeq2SeqModelOutput(
        ...
        if self.training:
            pvla_seq2seq_model_output.ot_loss = encoder_outputs.ot_loss
        return pvla_seq2seq_model_output
...
class BartForMultiModalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=None, dim_common=256, n_attn_heads=1, gamma=1e-5):
        super().__init__(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = masked_lm_loss + self.gamma * outputs.ot_loss
